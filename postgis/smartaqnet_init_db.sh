...
psql -v ON_ERROR_STOP=1 --username "${POSTGRES_USER}" --dbname "${POSTGRES_DB}" <<EOSQL
CREATE ROLE ${POSTGRES_TIMEOUT_USER} SUPERUSER CREATEDB CREATEROLE INHERIT LOGIN PASSWORD '${POSTGRES_TIMEOUT_USER_PASSWORD}';
ALTER ROLE ${POSTGRES_TIMEOUT_USER} SET statement_timeout=${POSTGRES_TIMEOUT_MILLISECONDS};
EOSQL
